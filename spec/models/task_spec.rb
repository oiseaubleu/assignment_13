...
RSpec.describe Task, type: :model do
  describe "バリデーション" do
    ...
    it "完了期限が未入力の場合、タスクのバリデーションが無効であること" do
      task = Task.new(title:"test2",description: "test2", status: :todo, deadline: nil)
      expect(task).to be_invalid
    ...
    it "完了期限が過去の日付の場合、タスクのバリデーションが無効であること" do
      task = Task.new(title:"test3",description: "test3", status: :todo, deadline: DateTime.now-1)
      expect(task).to be_invalid
    ...
    it "完了期限が今日の日付の場合、タスクのバリデーションが有効であること" do
      task = Task.new(title:"test4",description: "test4", status: :todo, deadline: DateTime.now)
